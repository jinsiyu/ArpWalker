...
def handle_fix_asset_stock_list_page(scraper, timeout=5):
    """
    处理固定资产列表网页的方法
    ...
    Returns:
        bool: 如果是固定资产列表并成功处理则返回True，否则返回False
    """
    try:
        # 查找页面上所有的链接
        ...
        for i, link in enumerate(links):
            ...
            if link.text == "已办结" or link.text == "已审批":
                ...
                handle_fix_asset_stock_page(scraper, timeout)
...
def main():
    ...
    while scraper.wait_for_user_interaction(
        ...
        # 用户点击了"是"，认为当前页面是固定资产列表页
        print("用户确认当前页面为固定资产列表页")
        # 尝试查找页面上的所有链接并输出
        handle_fix_asset_stock_list_page(scraper)
