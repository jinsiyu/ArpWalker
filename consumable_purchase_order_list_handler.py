...
def handle_consumable_purchase_order_list_page(scraper, timeout=5):
    """
    处理耗材列表网页的方法
    ...
    try:
        ...
        for i, link in enumerate(links):
            ...
            if link.text == "已办结" or link.text == "已审批":
                ...
                handle_consumable_purchase_order_page(scraper, timeout)
...
def main():
    ...
    while scraper.wait_for_user_interaction(
        ...
        # 用户点击了"是"，认为当前页面是耗材采购列表页
        print("用户确认当前页面为耗材采购列表页")
        # 尝试查找页面上的所有链接并输出
        handle_consumable_purchase_order_list_page(scraper)
    # 用户点击了"否"或关闭了对话框
    print("用户取消操作或认为当前页面不是耗材采购列表页")
