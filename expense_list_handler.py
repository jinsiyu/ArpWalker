...
def handle_expense_list_page(scraper, timeout=5):
    """
    处理报销凭证网页的方法
    ...
    Returns:
        bool: 如果是报销查询页并成功处理则返回True，否则返回False
    """
    try:
        # 查找页面上所有的链接
        ...
        for i, link in enumerate(links):
            ...
            if link.text == "已办结" or link.text == "已审批":
                ...
                handle_expense_page(scraper, timeout)
...
def main():
    ...
    while scraper.wait_for_user_interaction(
        ...
        # 用户点击了"是"，认为当前页面是报销查询页
        print("用户确认当前页面为报销查询页")
        # 尝试查找页面上的所有链接并输出
        handle_expense_list_page(scraper)
