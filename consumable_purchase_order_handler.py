...
def handle_consumable_purchase_order_page(scraper, timeout=5):
    ...
    try:
        ...
        if existing_folder:
            ...
            print(f"找到包含耗材采购单号的现有文件夹: {download_dir.absolute()}")
        else:
            # 构建包含所有单号的文件夹名
            folder_parts = []
            if purchase_order_number_text:
        ...
        # 点击订单打印元素
        a_list = scraper.find_elements(By.XPATH, "//a[text()='订单打印']")
        print(f"找到 {len(a_list)} 个订单打印:")
        ...
        # 获取采购单号并重命名下载的PDF文件
        if purchase_order_number_text:
            old_pdf_path = download_dir / "AM_MATERIAL_PURCHASE_ORDER.pdf"
            new_pdf_path = download_dir / f"耗材采购单号{purchase_order_number_text}.pdf"
            ...
            if old_pdf_path.exists():
                ...
                if final_new_path != new_pdf_path:
                    print(
                        f"已将 AM_MATERIAL_PURCHASE_ORDER.pdf 重命名为 {final_new_path.name} (原名称 {new_pdf_path.name} 已存在，使用新名称)")
                else:
                    print(f"已将 AM_MATERIAL_PURCHASE_ORDER.pdf 重命名为 {final_new_path.name}")
            else:
                print(f"警告: 未找到 AM_MATERIAL_PURCHASE_ORDER.pdf 文件，可能下载失败或名称不同")
...
def main():
    ...
    if user_confirmed:
    # 用户点击了"是"，认为当前页面是耗材采购单页
        print("用户确认当前页面为耗材采购单页")
        handle_consumable_purchase_order_page(scraper)
    else:
        # 用户点击了"否"或关闭了对话框
        print("用户取消操作或认为当前页面不是耗材采购单页")
